...
- Sampling occurred in March of 2012, but we will only use data from November surveys.
- The `scripts/data-wranging/Summarise-all-channels.R` script averages the quadrats by station, and then further averages into different categories:
  ...
  - Total plant cover
  - Herbaceous cover
- Grouping based on lifeform beyond just woody/herbaceous is not pursued anymore. Ground cover data also not used in later analysis, so it has been removed.
- `Year` is formatted in several ways:
  - As only 4 digits, as character (`20XX`)
  - As the correct year and month, in date format (`20XX-XX-01`)
    - 2021 sampling technically happened at the end of October, but we do not need to be as specific as the sampling day.
