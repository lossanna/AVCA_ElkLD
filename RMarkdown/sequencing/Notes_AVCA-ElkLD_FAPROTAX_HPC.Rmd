...
- Next four columns: sample names.  
- Last column (`taxonomy`, wrapped around to its own line): how taxonomy information be formatted for FAPROTAX input.
```{r}
ex[1, c(1, 16:20)]
```
...
# FAPROTAX
python3 collapse_table.py -i faprotax-input.tsv -o faprotax-output.tsv -r faprotax-report.txt -g FAPROTAX.txt -d "taxonomy" -v
```


### Run the job
- Access UA servers at <https://ood.hpc.arizona.edu/>.
- Run command: `sbatch /groups/egornish/lossanna/AVCA-ElkLD_16S/FAPROTAX_1.2.6/faprotax.slurm`
  - `sbatch`: submit a job
  - `/groups/...`: path to `SLURM` script.
- Note: if rerunning FAPROTAX, delete the `faprotax-output.tsv` file first (it cannot overwrite the existing file).
