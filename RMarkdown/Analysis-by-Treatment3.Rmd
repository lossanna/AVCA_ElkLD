...
- In comparing means from linear models with `Year` as a random factor, Control > Treated for total cover and herbaceous cover, and there was no significant difference for grass/forb/shrub cover. Control > Treated for perennial richness, and something weird happened with the Shannon diversity linear model.
...
### CV
- T-test for normally distributed and Wilcoxon test for non-normally distributed CV values. No significant differences between Control and Treated for any.
```{r, echo=FALSE, fig.dim=c(11,7)}
ggarrange(total.plot.cv, herb.plot.cv, notree.plot.cv,
          ncol = 2, nrow = 2)
```
```{r, echo=FALSE, fig.dim=c(11,3.5)}
...
```
```{r, echo=FALSE, fig.dim=c(11,7)}
ggarrange(total.plot.21, herb.plot.21, notree.plot.21,
          ncol = 2, nrow = 2)
```
```{r, echo=FALSE, fig.dim=c(11,3.5)}
ggarrange(rich.plot.21, shan.plot.21,
          ncol = 2, nrow = 1)
```
...
rich.pd <- read.csv("../data/cleaned/Percent-difference_rich.csv")
shan.pd <- read.csv("../data/cleaned/Percent-difference_shan.csv")
```
