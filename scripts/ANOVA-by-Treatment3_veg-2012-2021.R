...
# Repeat measures ANOVA
summary(aov(Cover ~ Treatment3 + Error(Year), data = annual.all)) # NS
summary(aov(Cover ~ Treatment3, data = annual.all)) # without repeat measures to compare

# Year as random factor
lm.annual <- lmer(Cover ~ Treatment3 + (1|Year), data = annual.all)
check_model(lm.annual) # this looks awful; not normally distributed
Anova(lm.annual) # NS

lm2.annual <- lme(Cover ~ Treatment3, random = ~1|Year, data = annual.all)
emmeans(lm2.total, specs = "Treatment3") 


# Two-factor ANOVA
summary(aov(Cover ~ Treatment3 * Year, data = annual.all))
# Treatment3:Year   5    233    46.6   2.164 0.0576 
check_model(aov(Cover ~ Treatment3 * Year, data = annual.all)) # this looks awful
...
# One-way ANOVA for Control
summary(aov(Cover ~ Year, data = filter(annual.all, Treatment3 == "Control")))
...
tiff("figures/2023-07_draft-figures/temporal-ANOVA_annual-cover.tiff", width = 8, height = 4, units = "in", res = 150)
annual.plot
dev.off()
